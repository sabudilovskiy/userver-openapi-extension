...
UTEST(openapi_validates, ArrayMinItems){
    std::vector<int> v;
    constexpr array_requirements req {
        .min_items = 1
    };
    auto validate_result = validate(v, uopenapi::utils::create_nttp_adl<req>());
    EXPECT_TRUE(validate_result.has_error());
    EXPECT_EQ(validate_result.error_message(), "array has size: [0] less than min_items: [1]");
}
...
UTEST(openapi_validates, ArrayMaxItems){
    ...
    auto validate_result = validate(v, uopenapi::utils::create_nttp_adl<req>());
    EXPECT_TRUE(validate_result.has_error());
    EXPECT_EQ(validate_result.error_message(), "array has size: [5] greater than max_items: [4]");
}
...
UTEST(openapi_validates, ArrayUniqueItems){
    ...
    auto validate_result = validate(v, uopenapi::utils::create_nttp_adl<req>());
    EXPECT_TRUE(validate_result.has_error());
    EXPECT_EQ(validate_result.error_message(), "array has non-unique items. first_index: [0], second_index: [5]");
}
