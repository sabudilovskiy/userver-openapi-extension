...
#error("unsupported compiler")
    ...
    constexpr auto get_full_name_type() {
        ...
        constexpr auto clear_name = details::get_clear_name<sizeof("uopenapi::utils::get_full_name_type")>(raw_name);
        return clear_name;
    }
    ...
    constexpr auto get_type_name_info() {
        ...
        constexpr std::size_t count_namespaces = std::count(name.begin(), open_template, ':') / 2;
    ...
    constexpr bool has_anonymous_namespace() {
        ...
        return std::find(info.namespaces.begin(), info.namespaces.end(), anonymous_ns) != info.namespaces.end();
    }

    template<typename T>
    constexpr bool is_template() {
        constexpr auto info = get_type_name_info<T>();
        return std::find(info.name.begin(), info.name.end(), '<') != info.name.end();
    }
}