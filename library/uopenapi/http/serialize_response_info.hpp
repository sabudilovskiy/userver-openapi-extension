...
response_info make_base_response_info() {
    ...
    if constexpr (count_bodies<T>() == 1) {
        return response_info{.content_type =
...
response_info serialize_response_info(const T& t) {
    ...
    auto visit_all = [&]<typename Info, typename F>(const F& f) {
        ...
        if constexpr (st == source_type::body_JSON) {
            res.body = details::serialize_json_body(f);
        } else if constexpr (st == source_type::cookie) {
            response_serializator<F, source_type::cookie>::serialize(
                f, res, Info::name.AsStringView());
        } else if constexpr (st == source_type::header) {
            response_serializator<F, source_type::header>::serialize(
                f, res, Info::name.AsStringView());
        } else if constexpr (st == source_type::query) {
            static_assert(st != source_type::query,
                          "queries cannot be used in responses");
        } else {
            static_assert(st == source_type::body_JSON, "unreachable");
        }
    };
