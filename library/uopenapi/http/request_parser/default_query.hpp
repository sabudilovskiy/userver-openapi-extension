...
struct request_parser<std::vector<F>, source_type::query> {
    ...
    static Field missing(std::string_view) { return {}; }
    static Field parse(const std::vector<std::string>& queries,
                       std::string_view) {
...
struct request_parser<Field, source_type::query> {
    static Field missing(std::string_view field_name) {
        throw utils::formatted_exception("Not founded queries with name: [{}]",
                                         field_name);
    }
    static Field parse(const std::vector<std::string>& queries,
...
struct request_parser<Field, source_type::query> {
    ...
    static Field parse(const std::vector<std::string>& queries,
        ...
        return utils::converter<std::string_view, raw_type>::convert(
            queries[0]);
    }
};
}  // namespace uopenapi::http