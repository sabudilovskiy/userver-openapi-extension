...
namespace uopenapi::http{
    ...
    struct response_serializator<T, source_type::header>{
        static void serialize(const T& t, response_info& r_info, std::string_view name){
            ...
            auto [it, emplaced] = r_info.headers.emplace(name, converter::convert(t));
            if (!emplaced){
                throw utils::formatted_exception("Detected duplicated field in headers: [{}]", name);
            }
        }
    };
    template <typename T> requires (utils::is_optional<T>)
    struct response_serializator<T, source_type::header>{
        static void serialize(const T& t, response_info& r_info, std::string_view name){
            ...
            auto [_, emplaced] = r_info.headers.emplace(name, converter::convert(getter::value(t)));
            if (!emplaced){
                throw utils::formatted_exception("Detected duplicated field in headers: [{}]", name);
            }
        }
