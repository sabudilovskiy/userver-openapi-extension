...
namespace uopenapi::pfr_extension{
    ...
    consteval auto get_sorted_names_with_indexes(){
        ...
        std::array<std::pair<std::string_view, std::size_t>, boost::pfr::tuple_size_v<T>> names_pair{};
        for (std::size_t i = 0; i < names.size(); i++) {
            names_pair[i] = {names[i], i};
        }
        std::sort(names_pair.begin(), names_pair.end(), [](const auto& left, const auto& right) {
    ...
    constexpr auto get_set_of_names(){
        constexpr auto result_with_nulls_with_size = details::get_result_with_nulls<T...>();
