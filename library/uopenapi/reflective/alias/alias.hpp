...
namespace uopenapi::reflective {
    ...
    struct Alias {
        ...
        template <typename...T> requires std::is_constructible_v<U, T...>
        ...
        template <typename T> requires std::is_assignable_v<U, T>
        auto& operator=(T&& t){
    ...
    template <typename T> requires std::is_base_of_v<Alias<typename T::basic_type>, std::remove_cvref_t<T>>
    ...
    struct alias_getter<T>{
        decltype(auto) get(T t){
           return utils::forward_like<T>(*t);
        }
    };

    template <typename T, auto req> requires is_alias_v<T>
    auto validate(T&& t) {
        using req_type = decltype(req);
        static_assert(std::is_same_v<req_type, none_requirements>, "alias must be without requirements on field");
        return validate<req>(alias_getter<T>(std::forward<T>(t)));
    }

    template <typename T, typename Requirements> requires is_alias_v<T>
    auto validate(T&& t, Requirements) {
