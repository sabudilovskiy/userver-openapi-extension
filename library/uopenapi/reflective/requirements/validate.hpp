...
is_validate_result auto field_call_validate(const F& f) {
    if constexpr (nttp_requirements_field<T, name> &&
        ...
        return validate(f,
                        utils::create_nttp_adl<requirements_field<T, name>>());
    } else if constexpr (nttp_requirements_field<T, name> &&
                         !field_exist_nttp_validate<T, name, F>) {
        static_assert(std::is_same_v<F, void> ||
                          std::is_same_v<requirements_field_t<T, name>, void>,
                      "не найдена перегрузка с передачей как nttp");
    } else if constexpr (field_exist_validate<T, name, F>) {
        return validate(f, requirements_field<T, name>);
    } else {
        static_assert(
            std::is_same_v<F, void> ||
                std::is_same_v<requirements_field_t<T, name>, void>,
